...
void TestStage::init()
{
	getAudioAPI().play(getResources().get<AudioClip>("Loveshadow_-_Marcos_Theme.ogg"), AudioPosition::makeUI(1.0f), 1.0f);
}
...
void TestStage::onVariableUpdate(Time time)
{
	...
	for (int i = 0; i < 12; ++i) {
		if (key->isButtonPressed(noteKeys[i])) {
			getAudioAPI().play(getResources().get<AudioClip>(noteSamples[i] + ".ogg"), AudioPosition::makeUI(1.0f), pan);
		}
	}
}
